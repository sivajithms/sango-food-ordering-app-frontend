...
const MobileTabs = () => {
  ...
  return (
    <>
<div className="md:hidden bg-white p-3 fixed bottom-0 z-10 w-full flex items-center justify-between border text-gray-500">          {allTypes.map((items) => (
            <Link to={`/${items.id}`}>
              <div
                className={
                  type === items.id
...
const LargeTabs = () => {
  ...
  return (
    <>
      <div className="md:px-4 lg:px-20 hidden md:flex gap-10 container my-8 mx-auto">
        {allTypes.map((items) => (
            ...
            >
              <div
                className={classnames(
                  ...
                  { [`bg-${items.activeColor}-700`]: type === items.id }
                )}
              >
                <img
                  src={
